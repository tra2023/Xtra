...
interface RecentEmotesDao {
    ...
    @Query("SELECT * FROM recent_emotes ORDER BY used_at DESC")
    suspend fun getAll(): List<RecentEmote>

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertAll(emotes: Collection<RecentEmote>)
    ...
    @Transaction
    suspend fun ensureMaxSizeAndInsert(emotes: Collection<RecentEmote>) {
        insertAll(emotes)
        deleteOld()
    }
}
